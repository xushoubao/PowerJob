...
import tech.powerjob.server.datax.admin.core.util.LocalCacheUtil;
import tech.powerjob.server.datax.admin.entity.JobDatasource;
import tech.powerjob.server.datax.admin.tool.database.ColumnInfo;
import tech.powerjob.server.datax.admin.tool.database.DasColumn;
import tech.powerjob.server.datax.admin.tool.database.TableInfo;
import tech.powerjob.server.datax.admin.tool.meta.DatabaseInterface;
import tech.powerjob.server.datax.admin.tool.meta.DatabaseMetaFactory;
import tech.powerjob.server.datax.admin.util.AESUtil;
import tech.powerjob.server.datax.admin.util.JdbcConstants;
import tech.powerjob.server.datax.admin.util.JdbcUtils;
        ...
        try {
            ...
            for (int i = 1; i <= columnCount; i++) {
                ...
                if (JdbcConstants.HIVE.equals(datasource)) {
                    if (columnName.contains(SPLIT_POINT)) {
                        res.add(i - 1 + SPLIT_SCOLON + columnName.substring(columnName.indexOf(SPLIT_POINT) + 1) + SPLIT_SCOLON + metaData.getColumnTypeName(i));
                    } else {
                        res.add(i - 1 + SPLIT_SCOLON + columnName + SPLIT_SCOLON + metaData.getColumnTypeName(i));
                    }
                } else {
                    res.add(columnName);
                }
